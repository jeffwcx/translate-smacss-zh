...
如果一个按钮需要在多个状态之间切换的话，那使用状态的命名约定可能会更有意义一点。
...
```
我喜欢将上面两个例子做比较，因为这显示了SMACSS是一个关于清晰和命名规范的理论。两种方式出现在项目里，我都乐于见得。下面让我们看另一个方法：属性选择器。
...
不过还是得说，表示状态可不止以上这些方法。

#### 基于类用CSS动画做状态改变

CSS动画是有趣而强大，但很多人可能认为它被定义在一个不应该被定义的层。毕竟，CSS是用来描述样式的，而JavaScript才是描述行为的。
...
不过，我必须承认的是，上述例子并不会起作用，因为当前的动画规范及浏览器实现并不允许我们在动画当中定义一个非动画的属性。幸运的是，规范仍然在变化，相信随着浏览器的实现，这个特性很快就会被补充进去。为了让我们例子起作用，我们需要像下面这样做：
...
如上述所说，我们可以使用类和属性处理模块上的状态变化。但是CSS同样提供了各种伪类来帮助我们进行状态的管理和变化。

从CSS2.1开始，最有用的三个伪类就是`:hover`，`:focus`，`:active`，这三个伪类能够响应用户操作。CSS3添加了很多新的伪类，大多数是基于HTML结构的样式（例如`:nth-child`或`:last-child`）。
...
最后的代码例子，我们编写了5个模块变体：一个基础模块，一个子模块，以及它们出现的3个伪类状态。**当我们基于类引入的状态越多，那么代码就会变得更复杂**。
...
幸好，在你的设计里很少有模块有如此复杂的状态管理。但是，需要明确的是，正确的样式管理会让你的项目更易维护。


### 通过媒体查询改变状态

虽然使用类和伪类进行状态改变已经相当平常了，但是媒体查询也正快速成为管理状态的另一条途径——往常只能通过JavaScript来实现的。适配设计和响应式Web设计使用媒体查询来响应各种标准。打印样式（Print Style Sheets）是其中一条媒体查询，允许我们定义页面打印时的样式。
